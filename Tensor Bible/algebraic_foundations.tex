...
In the complex case, if $A(\alpha \vb{v}) = \overline{\alpha} A(\vb{v})$, then $A$ is anti-linear. By convention, a linear operator's argument parentheses may be dropped: $T \vb{x} = T(\vb{x})$.
...
\begin{align*}
	w_{i} &= \vb{f}^{i*} \left( T(\vb{v}) \right) \\
	&= \vb{f}^{i*} \left( T \left( \sum_{j = 1}^{n} v^{j} \vb{e}_{j} \right) \right) \\
	&= \sum_{j = 1}^{n} \vb{f}^{i*} \left(T( \vb{e}_{j}) \right) v^{j} \\
	&= \sum_{j = 1}^{n} T_{ij} v^{j} \\
	T_{ij} &= \vb{f}^{i*} \left(T( \vb{e}_{j}) \right)
\end{align*}
