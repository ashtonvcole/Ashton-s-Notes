...
\section{Topological Functions}

The following functions are common, but not exhaustive, ways to create topologies on sets and vector spaces. They are very useful in continuum mechanics because they standardize distances, magnitudes, and angles outside of geometry.
...
\section{Dual Spaces and Functionals}

\begin{definition}[Algebraic Dual Space]
The algebraic dual $V^{*}$ of a vector space $V$ over a field $\mathbb{F}$ is the set of all linear functionals from $V$ to $\mathbb{F}$.
\begin{equation}
	V^{*} = L(V, \mathbb{F}) \subseteq \mathbb{F}^{V}
\end{equation}
\end{definition}

This is also a vector space over $\mathbb{F}$ (Proof \ref{proof:alg_dual_is_vs}).

\begin{definition}[Dual Set]
	The dual set $\{ \vb{e}^{i*} \}_{i \in I}$ of a vector space $V$ over $\mathbb{F}$, with respect to a certain basis $\{ \vb{e}_{i} \}_{i \in I}$, is a set of linear functionals generated by the following rule.
	\begin{equation*}
		\vb{e}^{i*}(\vb{e}_{j}) = \delta_{ij}
	\end{equation*}
\end{definition}

This set is linearly independent. It defines an injection from $V$ into $V^{*}$. If the space is finite-dimensional, then the dual set is a basis for the algebraic dual space.

\begin{definition}[Duality Pairing]
	The duality pairing between a vector space $V$ over $\mathbb{F}$ and its algebraic dual $V^{*}$ is a bilinear functional onto the field which evaluates the functional at that vector.
	\begin{equation}
		\langle f, \vb{x} \rangle_{X^{*}, X} = \langle \vb{x}, f \rangle_{X, X^{*}} = f(\vb{x})
	\end{equation}
\end{definition}

\begin{definition}[Topological Dual Space]
The topological dual $V'$ of a topological vector space $V$ over $\mathbb{F}$ is the set of all continuous linear functionals from $V$ to $\mathbb{F}$.
\begin{equation}
	V' = B(V, \mathbb{F}) \subseteq V^{*}
\end{equation}
\end{definition}

This is also a vector space over $\mathbb{F}$ (Proof \ref{proof:top_dual_is_vs}). It can be proven that $V^{*} = V'$ if and only if $V$ is finite-dimensional.

\section{Riesz Representation and Cobasis}

\begin{theorem}[Riesz Representation Theorem]
	Given a Hilbert space $H$ over $\mathbb{F}$, every continuous linear functional can be uniquely represented by an inner product.
	\begin{equation}
		\forall f \in H': \exists! \vb{u} \in H: \forall \vb{v} \in V: f(\vb{v}) = (\vb{v}, \vb{u})_{H}
	\end{equation}
\end{theorem}

\begin{definition}[Riesz Map]
	The Riesz map $R: H \leftrightarrow H'$ is an antilinear isometric isomorphism between a Hilbert space $H$ and its topological dual.
	\begin{equation}
		R \vb{u} = (\cdot, \vb{u})_{H}
	\end{equation}
\end{definition}

Note that $R$ is antilinear and $\vb{u}$ is in the second argument so that $R \vb{u}$ is linear, not bilinear. This nuance is sometimes lost for real vector spaces. In practice, the notation changes. In linear algebra, $R \vb{u} = \vb{u}^{*}$. In quantum mechanics, $R \ket{u} = \bra{u}$.

A nice visual correspondence can be established between the evaluation map and inner product.

\begin{equation}
	\langle \vb{v}, R \vb{u} \rangle_{H, H'} = (\vb{v}, \vb{u})_{H}
\end{equation}

Especially for real-valued Hilbert spaces, where complex conjugation can be ignored, $R\vb{u}$ is not distinguished from $\vb{u}$ and notation blurs. What adds to the confusion is that angle brackets are also used for the inner product. Consider this example for $L^{2}(\Omega)$ over $\mathbb{R}$.

\begin{equation*}
	\langle f, g \rangle_{(L^{2}(\Omega))', L^{2}(\Omega)} = \langle g, f \rangle_{L^{2}(\Omega), (L^{2}(\Omega))'} = (f, g)_{L^{2}(\Omega)} = (g, f)_{L^{2}(\Omega)} = \int_{\Omega} f(x) g(x) \dd{x}
\end{equation*}

\begin{definition}[Cobasis]
	A cobasis $\{ \vb{e}^{i} \}_{i \in I}$ of a given basis $\{ \vb{e}_{i} \}_{i \in I}$ of an inner product space $V$ is a bais which satisfies the following property.
	\begin{equation}
		(\vb{e}^{i}, \vb{e}_{j})_{H} = (\vb{e}_{j}, \vb{e}^{i})_{H} = \delta_{ij}
	\end{equation}
\end{definition}

The dual basis for $H'$ corresponds directly to the cobasis for $H$ (Proof \ref{proof:cobasis}).

\begin{equation}
	(\vb{e}^{i})^{*} = \vb{e}^{i*}
\end{equation}

Now any vector or functional can be written in two ways.

\begin{align}
	\vb{v} &= \sum_{i = 1}^{N} v^{i} \vb{e}_{i} = \sum_{i = 1}^{N} v_{i} \vb{e}^{i} \\
	\vb{v}^{*} = f &= \sum_{i = 1}^{N} f^{i} \vb{e}_{i}^{*} = \sum_{i = 1}^{N} f_{i} \vb{e}^{i*}
\end{align}

Starting from the basis representation of a vector, the following relationships apply between the components.

\begin{flalign}
	&& v_{i} &= \sum_{j = 1}^{N} v_{j} (\vb{e}_{j}, \vb{e}_{i})_{H} & \text{Proof \ref{proof:cobasis_component}} \label{equation:} \\
	&& f^{i} &= \overline{v^{i}} & \text{Proof \ref{proof:}} \label{equation:}\\
	&& f_{i} &= \sum_{j = 1}^{N} \overline{v_{j} (\vb{e}_{j}, \vb{e}_{i})_{H}} = \overline{v_{i}} & \text{Proof \ref{proof:}} \label{equation:}
\end{flalign}
...
The set of linear operators between vector spaces $V$ and $W$ is $L(V, W)$ (Proof \ref{proof:LVW}). It is a vector space over the same field. Given a finite-dimensional domain and range, a linear transformation can be represented with components with respect to a basis, similar to a vector or covector. This means that once bases have been chosen, linear transformations can be computed purely with components.
...
\begin{equation}
	\langle A^{*} f, g \rangle_{X^{*}, X} = \langle f, A g \rangle_{Y^{*}, Y}
\end{equation}

\begin{definition}[Hermitian Adjoint]
	The Hermitian adjoint $A': Y \rightarrow X$ of a linear operator $A: X \rightarrow Y$ is a linear map with the following definition.
	\begin{equation}
		A' = R_{X}^{-1} \circ A^{*} \circ R_{Y}
	\end{equation}
\end{definition}
...
\begin{equation}
	(A' \vb{u}, \vb{v})_{X} = (\vb{u}, A \vb{v})_{Y}
\end{equation}
...
Additional notations include $A^{\#}$, $A^{\dagger}$ for quantum mechanics, and $A^{T}$ for real-valued finite-dimensional linear algebra. The matrix transpose is a special case of the adjoint. For complex-valued finite-dimensional linear algebra, the Hermitian adjoint is the conjugate transpose.

\begin{flalign}
	&& (A')_{ij} &= \overline{A_{ij}} & \text{Proof \ref{proof:adjoint_comp}} \label{equation:adjoint_comp}
\end{flalign}
