...
define div grad and curl properly

associated with manifold, represented by set of points and metric
...
\begin{flalign}
	...
	&& \grad{\left( \mathcal{D} \vddot \vb{S} \right)} &= \dots & \text{Proof \ref{proof:grad_Tens_ddot_tens}} \label{equation:grad_Tens_ddot_tens} \\
	&& \grad{\left( \vb{S} \vddot \mathcal{D} \right)} &= \dots & \text{Proof \ref{proof:grad_tens_ddot_Tens}} \label{equation:grad_tens_ddot_Tens} \\
...
\begin{flalign}
	...
	&& \grad{\left( \mathcal{D} \vdot \vb{a} \right)} &= \dots & \text{Proof \ref{proof:grad_Tens_dot_vec}} \label{equation:grad_Tens_dot_vec} \\
	&& \grad{\left( \vb{y} \vdot \mathcal{D} \right)} &= \dots & \text{Proof \ref{proof:grad_vec_dot_Tens}} \label{equation:grad_vec_dot_Tens}
\end{flalign}
...
curl curl to grad div/ laplacian

\begin{flalign}
	&& \left( \Skew \Grad{\vb{u}} \right) \vb{v} &= \frac{1}{2} \left( \curl{\vb{u}} \right) \cross \vb{v}??? & \text{Proof \ref{proof:action_skew_grad_vec}} \label{equation:action_skew_grad_vec}
\end{flalign}
