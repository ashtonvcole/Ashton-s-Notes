...
\begin{definition}[Conjugate Space]
	The conjugate space $\overline{V}$ of a vector space $V$ over a field $\mathbb{F}$ is the same vector space, except that the scalar multiplication used is the complex conjugate of the one used for $V$.
	\begin{equation}
		\alpha \overline{*} \vb{v} = \overline{\alpha} * \vb{v}
	\end{equation}
\end{definition}

The conjugate space is sometimes used to manage linearity and anti-linearity of mathematical constructions. It should be used very carefully.
...
This is also a vector space over $\mathbb{F}$ (Proof \ref{proof:alg_dual_is_vs}). However, functionals are usually not bolded, to emphasize that they are functions with scalar outputs.
...
\section{Riesz Representation and Hilbert Space Bases}
...
Note that $\vb{u}$ is intentionally placed in the second argument, to make $R \vb{u}$ linear. As a result, $R$ itself is antilinear. This nuance is sometimes lost for real vector spaces. In practice, the notation changes. In linear algebra, $R \vb{u} = \vb{u}^{*}$. In quantum mechanics, $R \ket{u} = \bra{u}$.
...
Some might just write $(f, g)$ and let the reader infer the rest.

\begin{definition}[Cobasis]
	A cobasis $\{ \vb{e}^{i} \}_{i \in I}$ of a given basis $\{ \vb{e}_{i} \}_{i \in I}$ of an inner product space $H$ is a bais which satisfies the following property.
	\begin{equation}
...
Now any vector has four different representations.
...
\begin{flalign}
	&& v_{i} &= \sum_{j = 1}^{N} v^{j} (\vb{e}_{j}, \vb{e}_{i})_{H} & \text{Proof \ref{proof:cobasis_component}} \label{equation:} \\
...
\begin{definition}[Orthonormal Basis]
	An orthonormal basis $\{ \vb{\hat{e}}_{i} \}_{i \in I}$ is a bais which is its own cobasis.
\end{definition}

When using an orthonormal basis, the superscript-subscript distinction is often dropped.

\begin{equation}
	\vb{v} = \sum_{i = 1}^{N} v_{i} \vb{\hat{e}}_{i}
\end{equation}