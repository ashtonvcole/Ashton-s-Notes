...
\begin{definition}[Field]
	A field $\{ \mathbb{F}, +, \cdot \}$ is a set $\mathbb{F}$ with an addition-like operation $+: A \times A \rightarrow A$ and a multiplication-like operation $+: A \times A \rightarrow A$ which satisfy the following properties.
	\begin{enumerate}
...
\begin{definition}[Vector Space]
	...
	\begin{enumerate}
		...
		\begin{enumerate}
			...
			\begin{equation}
				\forall \alpha, \beta \in \mathbb{F}, \vb{x} \in X: (\alpha + \beta) * \vb{x} = \alpha * \vb{x} \boldsymbol{+} \beta * \vb{x}
			\end{equation}
...
\begin{flalign}
	&& \forall \vb{x} \in X: 0 * \vb{x} &= \vb{0} & \text{Proof \ref{proof:smult_zero}} \label{equation:smult_zero} \\
	&& \forall \vb{x} \in X: (-1) * \vb{x} &= -\vb{x} & \text{Proof \ref{proof:smult_inverse}} \label{equation:smult_inverse}
\end{flalign}
...
A vector space with an associated metric is called a metric space.

\begin{definition}[Norm]
	A norm $\norm{\cdot}: X \rightarrow [0, \infty)$ is a function that generalizes magnitude of vectors in a vector space $X$ over $\mathbb{F}$ by satisfying the following properties.
	\begin{enumerate}
		...
		\begin{equation}
			\forall \alpha \in \mathbb{F}, \vb{x} \in X: \norm{\alpha \vb{x}} = \abs{\alpha} \norm{\vb{x}}
		\end{equation}
		\item Triangle inequality
		\begin{equation}
			\forall \vb{x}, \vb{y} \in X: \norm{\vb{x} + \vb{y}} \leq \norm{\vb{x}} + \norm{\vb{y}}
		\end{equation}
	\end{enumerate}
\end{definition}
...
\begin{flalign}
	&& d(\vb{x}, \vb{y}) &= \norm{\vb{x} - \vb{y}} & \text{Proof \ref{proof:nls_is_ms}} \label{equation:nls_is_ms}
\end{flalign}

All norms are equivalent in finite-dimensional spaces. See Proof \ref{proof:norm_equivalence}.

\begin{definition}
	An inner product $(\cdot, \cdot): X \times X \rightarrow \mathbb{F}$ is a function that generalizes the similarity of vectors in a vector space $X$ over $\mathbb{F}$ by satisfying the following properties.
	\begin{enumerate}
		\item Positive-definiteness
		\begin{equation}
...
An inner product also allows for an abstract notion of angle.
